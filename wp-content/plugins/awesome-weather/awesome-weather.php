...
function awesome_weather_logic( $atts )
{
	...
	if($show_stats AND isset($today->main) )
	{
		...
		if( isset($today->main->humidity) ) $rtn .= '<div class="awe_humidty">' . __('độ ẩm:', 'awesome-weather') . " " . $today->main->humidity . '%</div>';
		if( $wind_speed AND $wind_direction) $rtn .= '<div class="awe_wind">' . __('gió:', 'awesome-weather') . ' ' .$wind_speed_obj['speed'] . $wind_speed_obj['text'] . ' ' .$wind_speed_obj['direction'] . '</div>';
		if( $today_high AND $today_low) $rtn .= '<div class="awe_highlow">' . __('Cao', 'awesome-weather') . ' ' . $today_high . ' &bull; ' . __('Thấp', 'awesome-weather') . ' ' . $today_low . '</div>';	
